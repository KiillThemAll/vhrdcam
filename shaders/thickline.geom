...
{
        return vec2( vertex.xy / vertex.w ) * winScale;
        //return vertex;
}
...
void main(void)
{
  ...
  if( dot(v0,v1) < -miterLimit ) {
	...
	if( dot(v0,n1) > 0 ) {
		...
		EmitVertex();
                ...
                //gl_FrontColor = gl_FrontColorIn[1];
                gl_Position = vec4( p1 / winScale, 0.0, 1.0 );
		EmitVertex();
		EndPrimitive();
	}
	else {
  ...
  //gsTexCoord = vec2(0, 0);
  //gl_FrontColor = gl_FrontColorIn[1];
  gl_Position = vec4( (p1 + length_a * miter_a) / winScale, 0.0, 1.0 );
  ...
  gl_Position = vec4( (p2 + length_b * miter_b) / winScale, 0.0, 1.0 );
  EmitVertex();
  //gsTexCoord = vec2(0, 1);
  //gl_FrontColor = gl_FrontColorIn[2];
  gl_Position = vec4( (p2 - length_b * miter_b) / winScale, 0.0, 1.0 );
